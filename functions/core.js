...
function execute(input,actor,op,data,opId,now=Date.now()){
  ...
  if(op==='configure'){
    ...
    if(op==='mode'){
      ...
      a.bids??={};a.bids[opId]={amount:dollars(n),discordId:actor.id,bidder:attendance.character,ts:now};a.currentBid=dollars(n);if(a.endsAt-now<30000){a.endsAt=now+30000;a.remaining=30;}
    }else if(op==='payWin'){
      ...
      const bids=Object.values(a.bids||{}).filter(b=>!b.retracted).sort((a,b)=>b.amount-a.amount||a.ts-b.ts);const win=bids[0];need(win&&win.discordId,'Winner has no Discord identity');need(actor.rl||actor.id===String(win.discordId),'Only winner or RL can pay');need(!banned(root,String(win.discordId)),'Winner banned');need(Number(win.amount)===Number(a.currentBid),'Winner price mismatch');
      s.gsPayments??={};need(!s.gsPayments[data.auctionId]||s.gsPayments[data.auctionId].status==='refunded','Item already paid');
      const total=positive(win.amount);const gold=!!data.gold;need(!gold||actor.rl&&s.settlementMode==='mixed'&&s.goldEnabled,'Gold is unavailable');
      transfer(root,gold?house:String(win.discordId),'_run_'+data.runId,total,opId,'bid',now,{...meta,auctionId:data.auctionId,reason:gold?'House GS backing confirmed gold receipt':'Won item'});
      s.gsPayments[data.auctionId]={status:'paid',owner:String(win.discordId),amount:dollars(total),method:gold?'gold':'gs',goldAmount:gold?dollars(total)*1000/s.usdPer1000:0,rateUsdPer1000:s.usdPer1000,paidAt:now,operation:opId};a.paid=true;result={amount:dollars(total)};
      ...
      if(p.method==='gold')need(data.goldReturned===true,'Confirm gold returned');transfer(root,'_run_'+data.runId,p.method==='gold'?house:p.owner,positive(p.amount),opId,'transfer',now,{...meta,reason:'Won-item refund',auctionId:data.auctionId});p.status='refunded';p.refundedAt=now;run.auctions[data.auctionId].paid=false;
    }else if(op==='lockCuts'){
      ...
      if(calc.managementCut)transfer(root,'_run_'+data.runId,house,positive(calc.managementCut),opId+'_management','cut',now,{...meta,lines:s.gsCutLines});
      result=calc;
    }else if(op==='payoutChoice'){
      const run=runFor(root,data.runId),r=run.settlement.raiders?.[data.raiderKey];need(r&&raiderId(run,data.raiderKey)===actor.id,'Select your own cut');need(!r.paid,'Payout already completed');need(['gs','gold'].includes(data.method),'Invalid method');need(data.method!=='gold'||run.settlement.settlementMode==='mixed'&&run.settlement.goldEnabled,'Gold is unavailable');r.gsPayoutMethod=data.method;
      ...
      const gold=r.gsPayoutMethod==='gold';let goldAmount=0;
      if(gold){need(s.settlementMode==='mixed'&&s.goldEnabled&&data.goldDelivered===true,'Confirm delivery of the gold payout');const pieces=take(root,'_run_'+data.runId,n);goldAmount=pieces.reduce((sum,p)=>sum+p.usd*1000/p.rateUsdPer1000,0);need(Number.isFinite(data.expectedGold)&&Math.abs(goldAmount-data.expectedGold)<.000001,'Gold quote changed; review before paying');entry(root,'_run_'+data.runId,opId,'cut',-n,{...meta,method:'gold',owner:r.gsOwner,goldAmount},now);houseAccount.usdReserved=dollars(units(houseAccount.usdReserved||0)-n);}
      else transfer(root,'_run_'+data.runId,r.gsOwner,n,opId,'cut',now,meta);
      r.gsCredited=r.gsCut;r.paid=true;r.paidAmount=r.gsCut;r.paidAt=now;r.submission={...(r.submission||{}),method:gold?'gold':'gs',status:'paid',discordId:r.gsOwner};r.payoutEvents??={};r.payoutEvents[opId]={type:'paid',method:gold?'gold':'gs',amount:dollars(n),goldAmount,createdAt:now,createdBy:actor.id};
